...
* Virtual user: a fake user implemented by Gatling as a message
...
Basic tests can consist of these three parts
...
*Note*: We are also giving our chain a label to make it easily identifiable in the output.
...
Now in the sample recording we've made there's one user. That's nice to run through the flow, but that will (hopefully) not expose any performance issues in your project.

Gatling supports two different workload models for users:
